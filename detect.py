...
#        print(end - start)
    ...
    def write(x, batches, results):
        ...
        ft.draw_text(image=img, pos=(c1[0], c1[1] + t_size[1]-7 ), text=label, text_size=20, text_color=[255,255,255])
        # cv2.putText(img, label, (c1[0], c1[1] + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1, [225,255,255], 1)
        cv2.imshow('hello',img)
        cv2.waitKey(3000)
        return img

    list1 = list(map(lambda x: write(x, im_batches, orig_ims), output))
    det_names = pd.Series(imlist).apply(lambda x: "{}/det_{}".format(args.det,x.split("/")[-1]))
    #print("det_names is {}\n\n\n\n".format(det_names))
    list2 = list(map(cv2.imwrite, det_names, orig_ims))
    #print(list2)
